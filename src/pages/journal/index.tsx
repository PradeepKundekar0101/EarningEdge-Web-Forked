...
const Index = () => {
  ...
  const [isEditJournalDrawerOpen, setIsEditJournalDrawerOpen] = useState(false);
  const [isJournalUpdated, setIsJournalUpdated] = useState(false);
  ...
  const uploadFileMutation = useMutation({
    ...
    onSuccess: (_, variables) =>
      message.success(`File ${variables.file.name} uploaded successfully`),
    onError: (_, variables) =>
      message.error(`Failed to upload file ${variables.file.name}`),
  });
  ...
  const updateJournalMutation = useMutation({
    mutationFn: async (journalData: any) => {
      const response = await api.put(`/journal/update/${selectedJournal}`, journalData);
      setIsEditJournalDrawerOpen(false);
      console.log("test journal update = ", response);
      return response.data;
    },
    onSuccess: () => {
      message.success("Journal added successfully");
  ...
  const handleEditJournal = (journal: string) => {
    setSelectedJournal(journal);
    setIsEditJournalDrawerOpen(true);
  };
      ...
      <section className="p-4 relative z-10">
        <div className="text-black mb-4">
          <h1 className="text-2xl text-slate-200">
            Hello {user.firstName}, {getGreeting()}!{" "}
          </h1>
        </div>
            ...
            <List.Item className="">
              <div className="journal-entry-item p-3 w-full flex justify-between items-center  border-darkStroke border-[0.6px] rounded-md  bg-darkSecondary">
                ...
                <div className="flex justify-center items-center space-x-2 mr-1 md:mr-3">
                  ...
                  <button
                    className="bg-white text-black rounded-md px-4 py-2"
                    onClick={() => handleEditJournal(entry._id)}
                  >
                    Edit
                  </button>
                </div>
        ...
        <Drawer
          ...
          open={isEditJournalDrawerOpen}
        >
          <div className="flex justify-end">
            <div className="flex w-1/4 gap-3">
              <button
                onClick={handleEditJournalCancel}
                className="bg-red-600 text-white w-full py-2 rounded-md hover:bg-slate-700 transition"
              >
                Cancel
              </button>
              <button
                // disabled={isJournalUpdated}
                onClick={handleUpdateJournal}
                className={`${
                  isJournalUpdated
                    ? "bg-green-600 cursor-pointer"
                    : "bg-gray-600 cursor-not-allowed"
                } text-white w-full py-2 rounded-md hover:bg-slate-700 transition`}
              >
                Update journal
              </button>
            </div>
            ...
            <div key={res.question._id}>
              <div>
                ...
                <input
                  ...
                  // value={res.answer}
                  value={
                    responses.find((r) => r.question === res.question._id)
                      ?.answer || res.answer
                  }
                  onChange={(e) => {
                    handleInputChange(res.question._id, e.target.value);
                    setIsJournalUpdated(true);
                  }}
                  // readOnly
                />
              </div>
        ...
        {/* <Drawer
          ...
          open={isEditJournalDrawerOpen}
          placement="bottom"
          onClose={() => setIsEditJournalDrawerOpen(false)}
          style={{ backgroundColor: "#13111c" }}
        >
          <h1 className="text-center text-slate-400 journalTypeHeading font-semibold text-xl mb-4">
            {journalType === "entry" ? "Pre" : "Post"} Trade Journal
